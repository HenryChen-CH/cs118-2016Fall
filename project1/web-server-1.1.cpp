            ...
            for (int fd = 0; fd <= maxFd; fd++) {
                if (FD_ISSET(fd, &readFds)) {
                    if (fd == listenFd) {
                        ...
                        HttpRequest request;
                        bool valid = request.parse(ss.str());
                        // cout << ss.str();
                        if (recLen > 0 && request.getConnectionState() != "close") {
                            ...
                            if (filesize > 0) {
                                ...
                                while (1) {
                                    ...
                                    // for (int i = 0; i < len1; i++) cout << buf[i];
                                    if (len1 == 0) break;
                                    int len2 = send(fd, buf, len1, 0);
                                    if (len2 < 0) {
                                        cerr << "send error\n";
                                        return 11;
                                    }
                        ...
                        if (request.getVersion() == "HTTP/1.0" || request.getConnectionState() == "close") {
                            close(fd);
                            FD_CLR(fd, &watchFds);
                            cout << "socket closed:" << fd << "\n";
                        }
                    }
                }
            }
