...
void TCB::send() {
    ...
    while (true) {
        ...
        if (len < 0) {
            ...
            retrByTimer = false;
            recv_packet.header.ackNo = ntohs(recv_packet.header.ackNo);
            if (recv_packet.header.ackNo == seqNo) {
                dupAckCount++;
                update_windows_size(false, retrByTimer);
                if (dupAckCount == 3) {
                    if (send_packet(buffer.front()->packet, buffer.front()->len) < 0) {
                        perror("fast retransmit fail");
                        exit(-1);
                    }
                    cout << "Sending packet " << buffer.front()->packet.header.seqNo << " " << wnd << " " << ssthresh << " Fast Retransmission" << endl;
                    // update_windows_size(false);
                    continue;
                }
            } else if (within_buffer(recv_packet.header.ackNo) || recv_packet.header.ackNo == (buffer.back()->packet.header.seqNo+buffer.back()->len)%(MAX_SEQ_NUM+1)){
                dupAckCount = 0;
                update_windows_size(false, retrByTimer);
            }
...
void TCB::update_windows_size(bool timeout, bool retrByTimer){
    uint16_t curWin;
    if(timeout){
        if(!retrByTimer)ssthresh=max(1/2*cwnd,INITIAL_WIN_SIZE);
        cwnd=INITIAL_WIN_SIZE+1024;
        curWin=min(rcvWin,cwnd);
        curWin=curWin-curWin%1024;
        // cout<<"Congestion Control:Timeout"<<endl;
    }
    else{
        if(dupAckCount==0){
            if(cwnd<=ssthresh){
                cwnd+=1024;
                // cout<<"Congestion Control:SS"<<endl;
            }
            else{
                cwnd+=1024*1024/cwnd;
                // cout<<"Congestion Control:CA"<<endl;
            }
        }
        if(dupAckCount==3){
            ssthresh=cwnd/2;
            cwnd=ssthresh+3*1024;
            // cout<<"Congestion Control:FR"<<endl;
        }
        if(dupAckCount>3){
            cwnd+=1024;
            // cout<<"Congestion Control:FR"<<endl;
